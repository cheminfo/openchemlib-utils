...
(function (global, factory) {
  ...
  function getHoseCodesFromDiastereotopicID(diastereotopicID, options = {}) {
    const OCL = getOCL();
    const molecule = OCL.Molecule.fromIDCode(diastereotopicID); // One of the atom has to be marked !
...
${indent}columns: ${this.columns}
  ...
  function inspectMatrix$1() {
    ...
    return `${this.constructor.name} {
${indent}[
${indentData}${inspectData$1(this, indentData)}
${indent}]
${indent}rows: ${this.rows}
${indent}columns: ${this.columns}
}`;
  ...
  const maxRows$1 = 15;
  const maxColumns$1 = 10;
  const maxNumSize$1 = 8;

  function inspectData$1(matrix, indent) {
    const {
      rows,
      columns
    } = matrix;
    const maxI = Math.min(rows, maxRows$1);
    const maxJ = Math.min(columns, maxColumns$1);
    const result = [];
    ...
    for (let i = 0; i < maxI; i++) {
      ...
      for (let j = 0; j < maxJ; j++) {
        line.push(formatNumber$1(matrix.get(i, j)));
      }
    ...
    if (maxJ !== columns) {
      result[result.length - 1] += ` ... ${columns - maxColumns$1} more columns`;
    }

    if (maxI !== rows) {
      result.push(`... ${rows - maxRows$1} more rows`);
    }
  ...
  function formatNumber$1(num) {
    ...
    if (numStr.length <= maxNumSize$1) {
      return numStr.padEnd(maxNumSize$1, ' ');
    }

    const precise = num.toPrecision(maxNumSize$1 - 2);

    if (precise.length <= maxNumSize$1) {
      return precise;
    }

    const exponential = num.toExponential(maxNumSize$1 - 2);
    const eIndex = exponential.indexOf('e');
    const e = exponential.substring(eIndex);
    return exponential.substring(0, maxNumSize$1 - e.length) + e;
  }
  ...
  class AbstractMatrix$1 {
    ...
    static checkMatrix(value) {
      return AbstractMatrix$1.isMatrix(value) ? value : new Matrix$1(value);
    }
  ...
  class Matrix$1 extends AbstractMatrix$1 {
    constructor(nRows, nColumns) {
      super();
  ...
  installMathOperations$1(AbstractMatrix$1, Matrix$1);
  ...
  function getConnectivityMatrix(molecule, options = {}) {
    ...
    if (options.pathLength) {
      result = floydWarshall(new Matrix$1(result)).to2DArray();
    }
  ...
  function getHoseCodesForPath(molecule, from, to, maxLength) {
    ...
    for (let sphere = 0; sphere <= 2; sphere++) {
      ...
      hoses.push({
        sphere,
        oclID: fragment.getCanonizedIDCode(OCL.Molecule.CANONIZER_ENCODE_ATOM_CUSTOM_LABELS)
      });
    }
          ...
          if (molecule.getAtomicNo(from) === fromAtomicNumber) {
            if (molecule.getAtomicNo(to) === toAtomicNumber) {
              ...
              if (pathLength >= minLength && pathLength <= maxLength) {
                atomsInfo[from].couplings.push(getHoseCodesForPath(molecule, from, to, pathLength, atomsInfo[to].oclID));
              }
            }
          }
        }
