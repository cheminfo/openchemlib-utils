...
(function (global, factory) {
  ...
  function tagAtom(molecule, iAtom) {
    molecule.setAtomCustomLabel(iAtom, "".concat(molecule.getAtomLabel(iAtom), "*"));
  ...
  function toDiastereotopicSVG(molecule) {
    ...
    svg = svg.replace(/Atom:[0-9]+"/g, function (value) {
      ...
      return "".concat(value, " data-diaid=\"").concat(diaIDs[atom].join(','), "\"");
    });
    return svg;
  }
  ...
  function getHoseCodesFromDiastereotopicID(diastereotopicID) {
    let options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
    const OCL = getOCL();
